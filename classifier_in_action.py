...
def getUnTaggedEmails():
      sql = """
      SELECT body, datetime
      FROM email_tags
      WHERE tag IS NULL
      """
...
def saveClassifiedTag(tag, date):
    sql = """
    INSERT INTO sentiment_timeline(tag, datetime)
    VALUES (%s, %s);
    """

    cursor = db.cursor()
    cursor.execute(sql, [tag, date])
    db.commit()
...
emails = getUnTaggedEmails()
processedCount = 0
for body, date in emails:
    features = bodyToFeatureset(body, interesting_words)
    tag = classifier.classify(features)
    saveClassifiedTag(tag, date)
